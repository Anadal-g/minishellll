...
/*   Updated: 2025/07/25 13:20:31 by carolinamc       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		dup_builtin_fds(int fd_in, int fd_out, int *saved_stdin,
				int *saved_stdout);
void		restore_builtin_fds(int saved_stdin, int saved_stdout);
int			open_builtin_fds(t_token *token, t_env **env, int *fd_in,
				int *fd_out);
void		close_pipe(int *fd);
void		close_child_pipes(t_child_io_context *ctx);
pid_t		handle_cmd_iteration(t_token *current, t_env **env, int *prev_fd);
